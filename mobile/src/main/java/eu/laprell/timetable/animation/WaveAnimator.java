...
public class WaveAnimator extends CAnimator {
    ...
    public void start() {
        synchronized (mLock) {
            ...
            final int[] loc1 = new int[2];

            for(int i = 0;i < mTargets.size();i++) {
                Holder h = mTargets.get(i);

                AnimUtils.getViewLoc(h.target, loc1);

                double distance1;

                if (mComputeMoreExactly) {
                    distance1 = getDistanceBetweenMoreExaclty(h.target, loc1, mStartPoint);
                } else {
                    distance1 = getDistanceBetween(loc1, mStartPoint);
                }
     ...
     */
    private double getDistanceBetweenMoreExaclty(View a, int[]loc, int[]b) {
        AnimUtils.getViewLoc(a);
        ...
        // ROW 1
        xl = getDistanceBetween(repositionBy(p, loc,     0,     0), b);
        xm = getDistanceBetween(repositionBy(p, loc, w / 2,     0), b);
        xr = getDistanceBetween(repositionBy(p, loc,     w,     0), b);
        min_r1 = Math.min(xr, Math.min(xl, xm));

        // ROW 2
        xl = getDistanceBetween(repositionBy(p, loc,     0, h / 2), b);
        xm = getDistanceBetween(repositionBy(p, loc, w / 2, h / 2), b);
        xr = getDistanceBetween(repositionBy(p, loc,     w, h / 2), b);
        min_r2 = Math.min(xr, Math.min(xl, xm));

        // ROW 3
        xl = getDistanceBetween(repositionBy(p, loc,     0,     h), b);
        xm = getDistanceBetween(repositionBy(p, loc, w / 2,     h), b);
        xr = getDistanceBetween(repositionBy(p, loc,     w,     h), b);
        min_r3 = Math.min(xr, Math.min(xl, xm));

        return Math.min(Math.min(min_r1, min_r2), min_r3);
    }

    private int[] repositionBy(int[]p, int[] pos, int deltax, int deltay) {
        p[0] = pos[0] + deltax;
        p[1] = pos[1] + deltay;
